...
static void ext_session_shm_format(void *data,
	...
	wl_array_for_each(fmt, &cast->pending_constraints.shm_formats) {
		if (fmt->fourcc == fourcc) {
			logprint(TRACE, "ext: skipping duplicated format %u", format);
			return;
		}
	}
	if (xdpw_bpp_from_drm_fourcc(fourcc) <= 0) {
		logprint(WARN, "ext: unknown shm format: %d: stride not known", fourcc);
		return;
	}
	...
	logprint(TRACE, "ext: shm_format handler");
}
...
static void ext_session_dmabuf_format(void *data,
	...
	wl_array_for_each(modifier, modifiers) {
		...
		if (!new) {
			logprint(TRACE, "ext: skipping duplicated format %u (%lu)", format, *modifier);
			continue;
		}
		...
		logprint(TRACE, "ext: dmabuf_format handler: %d, %ld", format, *modifier);;
	}
